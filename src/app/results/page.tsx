          ...
          <section key={date} className="mb-8">
            ...
            <div className="space-y-2">
              {matches.map((match) => {
                ...
                return (
                  <div key={match.id} className="grid grid-cols-[120px_1fr_60px] items-center rounded-xl border border-border bg-bg-card px-5 py-3.5 transition-all hover:bg-bg-card-hover hover:border-border-hover cursor-pointer">
                    <div>
                    ...
                    <div className="flex items-center justify-center gap-4">
                      <div className="flex items-center gap-2 min-w-[140px] justify-end">
                        ...
                        <span className="h-5 w-5 rounded shrink-0" style={{ background: `${match.team1.color}66` }} />
                      </div>
                      <div className="flex items-center gap-2 rounded-lg bg-bg-surface px-3 py-1 min-w-[64px] justify-center">
                        <span className={`text-sm tabular-nums ${t1Won ? "font-bold text-blue-light" : "text-text-muted"}`}>{match.score1}</span>
                        <span className="text-text-muted text-xs">–</span>
                        <span className={`text-sm tabular-nums ${!t1Won ? "font-bold text-blue-light" : "text-text-muted"}`}>{match.score2}</span>
                      </div>
                      <div className="flex items-center gap-2 min-w-[140px]">
                        <span className="h-5 w-5 rounded shrink-0" style={{ background: `${match.team2.color}66` }} />
